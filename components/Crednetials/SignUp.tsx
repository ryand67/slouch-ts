    ...
    return (
        <div className="w-full">
            <form onSubmit={(e) => handleSignUp(e)} className="flex flex-col w-full">
                ...
                <input className="border border-gray-300 rounded p-2 focus:border-yellow-300" onChange={(e) => setName(e.target.value)} placeholder="Name" type="text" name="name" id="" />
                ...
                <input onChange={(e) => setConfirmPassword(e.target.value)} className="border border-gray-300 rounded p-2 focus:border-yellow-300" type="password" placeholder="Confirm Password" name="passwordRepeat" id="" />
                <p onClick={() => changeCredState(true)} className="text-center my-6">Already have an account? <span className="cursor-pointer underline text-yellow-400">Sign in!</span></p>
                <button onClick={(e) => handleSignUp(e)} className="rounded bg-yellow-500 w-44 mx-auto py-3 text-white hover:bg-white hover:text-yellow-300 transition-all border-4 border-yellow-300" type="submit">Sign Up</button>
            </form>
        </div>
    )
}
